...
- Official evaluation engine for trimester 1-6 and trimester 7-9 rules
- Configuration-driven Grade Lab generated from `data/assessment-rules.json`
- Course-specific assessment definitions generated from `data/course-assessments.json`
...
- Configurable assessment types, individual maximum marks, weightages, and best-of selection counts
...
Assessment types, weightages, trimester mappings, and best-of values are generated from `data/assessment-rules.json`. Individual course assessment IDs and maximum marks are generated from `data/course-assessments.json`. Local administrator edits are saved in Local Storage and can be exported as JSON, so future assessment rule changes can be handled by replacing JSON files without changing JavaScript.
