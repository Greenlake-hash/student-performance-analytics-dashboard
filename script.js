...
const STORAGE_KEY = "iitg-spad-state-v1";
const THEME_KEY = "iitg-spad-theme";
...
async function init() {
  try {
    const [courses, syllabus, assessmentRules] = await Promise.all([
...
function wireEvents() {
  ...
  $("#profileForm").addEventListener("submit", (event) => {
    ...
    state.profile = {
      name: $("#studentName").value.trim() || "IITG Student",
      studentId: $("#studentId").value.trim() || "IITG-DSAI",
  ...
  $("#loadTemplateBtn").addEventListener("click", () => {
    ...
    syncActivePlanFromRules({ preserveScores: true });
    persistState();
    showToast("Official JSON rules reloaded.");
  });
  ...
  $("#resetPlanBtn").addEventListener("click", () => {
    ...
    renderComponentRows();
    persistState();
    showToast("Active scores reset to the current rule set.");
  });
...
function populateProfile() {
  $("#profileChip").textContent = `${state.profile.name || "IITG Student"} - Target ${formatNumber(state.profile.targetCgpa, 1)} CGPA`;
  $("#studentName").value = state.profile.name || "";
  $("#studentId").value = state.profile.studentId || "";
  $("#targetCgpa").value = state.profile.targetCgpa ?? 8.5;
}
...
function planFromRule(rule, previousPlan = []) {
  const enabledComponents = (rule?.components || []).filter((component) => component.enabled !== false);
  return enabledComponents.map((component, index) => {
    const type = normalizeAssessmentType(component.type || component.name || `A${index + 1}`);
    const existing = findMatchingPlanComponent(previousPlan, type, index);
    const count = Math.max(1, Math.round(Number(component.count) || 1));
    const bestOf = clamp(Math.round(Number(component.bestOf) || count), 1, count);
    const maxMarks = Math.max(1, Number(component.maxMarks) || 1);
    return {
      id: existing?.id || component.id || makeId(),
      type,
      name: component.name || `${type} Scores`,
      count,
      bestOf,
      maxMarks,
      maxMarksList: normalizeMaxMarksList(existing?.maxMarksList, count, maxMarks),
      weight: clamp(Number(component.weight) || 0, 0, 100),
      scores: normalizeScoreList(existing?.scores, count)
    };
  });
}
  ...
  $("#ruleManagerRows").innerHTML = rows.map((component, index) => `
    <article class="rule-editor-card">
      ...
      <div class="rule-editor-grid">
        <label>
          Assessment Type
          <input data-rule-index="${index}" data-rule-field="type" value="${escapeAttr(component.type)}" aria-label="Assessment type">
        </label>
        <label>
          Count
          <input data-rule-index="${index}" data-rule-field="count" type="number" min="1" step="1" value="${Number(component.count) || 1}" aria-label="Assessment count">
        </label>
        <label>
          Best Of
          <input data-rule-index="${index}" data-rule-field="bestOf" type="number" min="1" step="1" value="${Number(component.bestOf) || Number(component.count) || 1}" aria-label="Best-of count">
        </label>
        <label>
          Maximum Marks
          <input data-rule-index="${index}" data-rule-field="maxMarks" type="number" min="1" step="0.5" value="${Number(component.maxMarks) || 1}" aria-label="Maximum marks">
        </label>
        <label>
          Weight
          <input data-rule-index="${index}" data-rule-field="weight" type="number" min="0" max="100" step="0.1" value="${Number(component.weight) || 0}" aria-label="Weight percent">
        </label>
      </div>
      <button class="danger-btn small-btn" type="button" data-remove-rule="${index}">Remove Type</button>
    </article>
  `).join("") || `<div class="empty-state">No components configured for this trimester.</div>`;
...
function updateAssessmentComponent(event) {
  ...
  if (!rule?.components?.[index]) return;
  const component = rule.components[index];
  const value = event.target.type === "checkbox" ? event.target.checked : event.target.value;
  component[field] = ["count", "bestOf", "maxMarks", "weight"].includes(field) ? Number(value) : value;
  if (field === "enabled") component.enabled = Boolean(value);
  if (field === "type") {
    component.type = normalizeAssessmentType(value);
    component.name = `${component.type} Scores`;
    ensureComponentDefinition(component.type);
  }
  component.count = Math.max(1, Math.round(Number(component.count) || 1));
  component.bestOf = clamp(Math.round(Number(component.bestOf) || component.count), 1, component.count);
  component.maxMarks = Math.max(1, Number(component.maxMarks) || 1);
  component.weight = clamp(Number(component.weight) || 0, 0, 100);
  syncActivePlanFromRules({ preserveScores: true });
  persistState();
}
...
function addAssessmentComponent() {
  const rule = getActiveRule();
  if (!rule.components) rule.components = [];
  const type = nextCustomAssessmentType(rule.components);
  rule.components.push({
    id: makeId(),
    type,
    name: `${type} Scores`,
    count: 1,
  ...
  syncActivePlanFromRules({ preserveScores: true });
  persistState();
  showToast(`${type} added to the active rule set.`);
}

function removeAssessmentComponent(index) {
  const rule = getActiveRule();
  if (!rule?.components?.[index]) return;
  const removed = rule.components.splice(index, 1)[0];
  syncActivePlanFromRules({ preserveScores: true });
  persistState();
  showToast(`${removed.type || "Assessment"} removed from the active rule set.`);
}
    ...
    rules: rules.map((rule, ruleIndex) => {
      ...
      return {
        ...
        components: (rule.components || []).map((component, componentIndex) => {
          const type = normalizeAssessmentType(component.type || component.name || `A${componentIndex + 1}`);
          const count = Math.max(1, Math.round(Number(component.count) || 1));
          return {
            ...
            name: component.name || `${type} Scores`,
            count,
            bestOf: clamp(Math.round(Number(component.bestOf) || count), 1, count),
            maxMarks: Math.max(1, Number(component.maxMarks) || 1),
            weight: clamp(Number(component.weight) || 0, 0, 100),
...
function setScoreAt(component, index, value) {
  ...
  scores[index] = Number.isFinite(parsed) ? parsed : null;
  component.scores = scores;
}

function setMaxMarksAt(component, index, value) {
  const count = Math.max(1, Number(component.count) || 1);
  const fallback = Math.max(1, Number(component.maxMarks) || 1);
  const marks = normalizeMaxMarksList(component.maxMarksList, count, fallback);
  marks[index] = Math.max(1, Number(value) || fallback);
  component.maxMarksList = marks;
}

function nextCustomAssessmentType(components) {
  const existing = new Set((components || []).map((component) => normalizeAssessmentType(component.type)));
  let index = 1;
...
function renderAssessmentCards(result) {
  ...
  $("#assessmentCardGroups").innerHTML = result.components.map((component, componentIndex) => {
    ...
    const selectedMax = component.selected.reduce((sum, item) => sum + item.maxMarks, 0) || 1;
    return `
      <section class="assessment-group" aria-labelledby="assessmentGroup${componentIndex}">
        <div class="assessment-group-head">
          <div>
            ...
            <p>${component.bestOf < component.count ? `Best ${component.bestOf} of ${component.count} selected automatically` : `All ${component.count} assessment${component.count === 1 ? "" : "s"} used`}</p>
          </div>
          <span class="weight-badge">${formatNumber(component.weight, 1)}% Weight</span>
        </div>
        <div class="assessment-card-grid">
          ${component.items.map((item) => {
            const status = selectedKeys.has(item.key) ? "included" : droppedKeys.has(item.key) ? "dropped" : "pending";
            const itemContribution = selectedKeys.has(item.key) && item.entered
              ? (item.score / selectedMax) * component.weight
              : 0;
            const sourceComponent = plan[componentIndex];
            const maxValue = getMaxMarksAt(sourceComponent, item.index);
            return `
              <article class="assessment-card ${status}" data-status="${status}">
                ...
                <label>
                  ...
                  <input
                    type="number"
                    min="0"
                    max="${escapeAttr(maxValue)}"
                    step="0.5"
                    value="${item.entered ? escapeAttr(item.score) : ""}"
                    data-score-component="${componentIndex}"
                    data-score-index="${item.index}"
                ...
                <label>
                  ...
                  <input
                    ...
                    data-max-component="${componentIndex}"
                    data-max-index="${item.index}"
                    ${state.beginnerMode ? "readonly" : ""}
                    aria-label="${escapeAttr(item.label)} maximum marks">
                </label>
                <div class="assessment-meta">
                  <span>Weight Contribution</span>
                  <strong>${formatNumber(itemContribution, 2)}%</strong>
                </div>
                <div class="best-status ${status}">
                  ${statusIcon(status)} ${status === "included" ? "Included" : status === "dropped" ? "Dropped" : "Pending"}
                </div>
              </article>
            `;
          }).join("")}
  ...
  $$("[data-score-component]", $("#assessmentCardGroups")).forEach((input) => {
    input.addEventListener("input", updateAssessmentScore);
  });
  $$("[data-max-component]", $("#assessmentCardGroups")).forEach((input) => {
...
function renderRuleExplanation(result) {
  ...
  const selectionPills = result.components.map((component) => {
    const selection = component.bestOf < component.count
  ...
  const weightPills = result.components.map((component) => `
    <button class="rule-pill" type="button" title="${escapeAttr(TERM_DEFINITIONS.Weight)}">${escapeHtml(component.type)} Weight = ${formatNumber(component.weight, 1)}%</button>
  `);
...
function updateAssessmentScore(event) {
  const course = getSelectedCourse();
  const component = state.assessmentPlans[course.code]?.[Number(event.target.dataset.scoreComponent)];
  if (!component) return;
  setScoreAt(component, Number(event.target.dataset.scoreIndex), event.target.value);
  persistState();
  renderCalculatorSummary();
}
...
function updateAssessmentMaxMarks(event) {
  const course = getSelectedCourse();
  const component = state.assessmentPlans[course.code]?.[Number(event.target.dataset.maxComponent)];
  if (!component) return;
  setMaxMarksAt(component, Number(event.target.dataset.maxIndex), event.target.value);
  persistState();
  renderCalculatorSummary();
}
...
function evaluatePlan(plan) {
  const components = plan.map((component, componentIndex) => evaluateComponent(component, componentIndex));
  return components.reduce((acc, component) => {
    acc.components.push(component);
    acc.projected += component.contribution;
...
function evaluateComponent(component, componentIndex) {
  const type = inferComponentType(component);
  const count = Math.max(1, Number(component.count) || 1);
  const bestOf = clamp(Math.round(Number(component.bestOf) || count), 1, count);
  const weight = clamp(Number(component.weight) || 0, 0, 100);
  const parsedScores = normalizeScoreList(component.scores, count);
  const items = Array.from({ length: count }, (_, index) => {
    const maxMarks = getMaxMarksAt(component, index);
    const rawScore = parsedScores[index];
    const entered = Number.isFinite(rawScore);
    const score = entered ? clamp(rawScore, 0, maxMarks) : 0;
    return {
      componentIndex,
      index,
      key: `${componentIndex}-${index}`,
      label: `${type}${index + 1}`,
      score,
      maxMarks,
      entered,
  ...
  const ranked = [...items].sort((a, b) => b.percentage - a.percentage || a.index - b.index);
  const selected = ranked.slice(0, bestOf).sort((a, b) => a.index - b.index);
  const dropped = ranked.slice(bestOf).sort((a, b) => a.index - b.index);
  const selectedScore = selected.reduce((sum, item) => sum + item.score, 0);
  const selectedMax = selected.reduce((sum, item) => sum + item.maxMarks, 0);
  const percentage = selectedMax ? (selectedScore / selectedMax) * 100 : 0;
  const contribution = percentage * (weight / 100);
  const enteredCount = items.filter((item) => item.entered).length;
  const attemptedWeight = count ? (enteredCount / count) * weight : 0;
  ...
  return {
    ...
    count,
    bestOf,
    maxMarks: Math.max(...items.map((item) => item.maxMarks), Number(component.maxMarks) || 1),
    weight,
    items,
    selected,
    dropped,
    ...
    contribution,
    attemptedWeight,
    remainingWeight: Math.max(0, weight - attemptedWeight)
  };
}
  ...
  $("#officialBreakdown").innerHTML = result.components.map((component) => `
    <div class="engine-card">
      ...
      <dl>
        ...
        <dt>Rule</dt><dd>Best ${component.bestOf} of ${component.count}</dd>
        <dt>Selected</dt><dd>${formatItems(component.selected)}</dd>
        <dt>Dropped</dt><dd>${component.dropped.length ? formatItems(component.dropped) : "None"}</dd>
      </dl>
    </div>
  `).join("");
}
...
function findRequiredNextAssessmentScore(targetPercentage) {
  ...
  const plan = state.assessmentPlans[course.code] || [];
  for (let componentIndex = 0; componentIndex < plan.length; componentIndex += 1) {
    const component = plan[componentIndex];
    const count = Math.max(1, Number(component.count) || 1);
    const scores = normalizeScoreList(component.scores, count);
    const missingIndex = scores.findIndex((score) => !Number.isFinite(score));
    if (missingIndex < 0) continue;
    const maxMarks = getMaxMarksAt(component, missingIndex);
    for (let score = 0; score <= maxMarks; score += 0.5) {
      const cloned = clonePlanForSimulation(plan);
      const clonedScores = normalizeScoreList(cloned[componentIndex].scores, count);
      clonedScores[missingIndex] = score;
      cloned[componentIndex].scores = clonedScores;
      const projected = evaluatePlan(cloned).projected;
      if (projected >= targetPercentage) {
        const type = inferComponentType(component);
        return { label: `${type}${missingIndex + 1}`, score, maxMarks };
      }
    }
  }
  return null;
...
function clonePlanForSimulation(plan) {
  return plan.map((component) => ({
    ...component,
    scores: normalizeScoreList(component.scores, Number(component.count) || 0),
    maxMarksList: Array.isArray(component.maxMarksList) ? [...component.maxMarksList] : []
  }));
}
...
function exportJson() {
  const payload = {
    project: "IITG Student Performance Analytics Dashboard",
    version: "1.0.0",
  ...
  downloadBlob(blob, `iitg-performance-${dateStamp()}.json`);
...
async function exportPdf() {
  ...
  try {
    ...
    pdf.setFontSize(14);
    pdf.text("IITG Student Performance Analytics Dashboard", 10, position);
    position += 8;
    ...
    pdf.save(`iitg-dashboard-report-${dateStamp()}.pdf`);
...
function chartToSvg(id, chart) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="960" height="540" viewBox="0 0 960 540">
    ...
    <text x="48" y="104" fill="#2dd4bf" font-family="Arial, sans-serif" font-size="16">SVG summary export generated by IITG Analytics</text>
    ${rows}
  </svg>`;
}
