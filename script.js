  ...
  $("#courseAssessmentRows").innerHTML = definitions.map((assessment, index) => `
    <article class="rule-editor-card course-assessment-editor">
      ...
      <div class="rule-editor-grid">
        ...
        <label>
          Weight %
          <input data-course-assessment-index="${index}" data-course-assessment-field="weight" type="number" min="0" max="100" step="0.1" value="${Number(assessment.weight) || 0}" aria-label="${escapeAttr(assessment.id)} weight percent">
        </label>
        <label class="mini-toggle">
      ...
      <section class="assessment-group" aria-labelledby="assessmentGroup${componentIndex}">
        <div class="assessment-group-head">
          <div>
            ...
            <p>${component.bestOf < component.eligibleCount ? `Best ${component.bestOf} of ${component.eligibleCount} eligible assessment${component.eligibleCount === 1 ? "" : "s"} selected automatically` : `All ${component.count} assessment${component.count === 1 ? "" : "s"} used`}</p>
          </div>
          <span class="weight-badge">${formatNumber(component.weight, 1)}% Active Weight</span>
        </div>
        <div class="assessment-card-grid">
          ${component.items.map((item) => {
            ...
            return `
              <article class="assessment-card ${status} ${item.validationErrors.length ? "invalid" : ""}" data-status="${status}">
                ...
                ${item.validationErrors.map((message) => `<div class="validation-message">${escapeHtml(message)}</div>`).join("")}
