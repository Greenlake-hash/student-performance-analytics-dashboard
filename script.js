...
const viewTitles = {
  ...
  portfolio: "Project Architecture & Portfolio Case Study",
...
async function init() {
  try {
    const [courses, syllabus] = await Promise.all([
...
function wireEvents() {
  ...
  $("#recordTrimester").addEventListener("change", renderCalculatorSummary);
  $("#targetInput").addEventListener("input", renderCalculatorSummary);
  $("#whatIfGradeSelect").addEventListener("change", renderCalculatorSummary);
  ...
  $("#loadTemplateBtn").addEventListener("click", () => {
    ...
    if (!course) return;
    state.assessmentPlans[course.code] = officialPlanForTrimester(Number($("#recordTrimester").value || course.trimester));
    renderComponentRows();
    persistState();
    showToast("Official IITG template loaded.");
  });
  $("#addComponentBtn").addEventListener("click", addAssessmentComponent);
  $("#resetPlanBtn").addEventListener("click", () => {
    const course = getSelectedCourse();
    if (!course) return;
    delete state.assessmentPlans[course.code];
    loadCoursePlan();
    persistState();
  ...
  $("#duplicatePlanBtn").addEventListener("click", () => {
    persistState();
    showToast("Template saved to Local Storage.");
  });
...
function populateStaticControls() {
  ...
  $("#whatIfGradeSelect").innerHTML = activeGradeScale()
    .filter((grade) => grade.letter !== "F")
...
function renderRecommendations() {
  ...
  recommendations.push({
    ...
    body: "Use the Grade Lab after every PT, NPT, ST, lab, or project update. Forecasting improves when each component has current scores."
  });
...
function renderAssessmentCharts() {
  ...
  const result = calculatePlanResult();
  const colors = chartColors();
  const componentByType = (type) => result.components.find((component) => component.type === type);
  const pt = componentByType("PT");
  const npt = componentByType("NPT");
  const st = componentByType("ST");

  drawTrendChart("ptTrendChart", pt, colors.brand, "PT Marks");
  drawTrendChart("nptTrendChart", npt, colors.amber, "NPT Marks");

  const comparisonComponents = result.components.filter((component) => ["PT", "NPT", "ST"].includes(component.type));
  drawChart("assessmentComparisonChart", {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: [colors.brandSoft, colors.amberSoft, colors.roseSoft],
        borderColor: [colors.brand, colors.amber, colors.rose],
        borderWidth: 1
      }]
    },
  ...
  drawChart("contributionChart", {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: [colors.brand, colors.amber, colors.rose],
        borderColor: colors.surface,
        borderWidth: 2
      }]
    },
...
function loadCoursePlan() {
  const course = getSelectedCourse();
  if (!course) return;
  if (!state.assessmentPlans[course.code] || !hasOfficialComponents(state.assessmentPlans[course.code])) {
    state.assessmentPlans[course.code] = officialPlanForTrimester(course.trimester);
  }
...
function hasOfficialComponents(plan) {
  return Array.isArray(plan) && plan.some((component) => ["PT", "NPT", "ST"].includes(inferComponentType(component)));
}
...
function clonePlan(plan) {
  return plan.map((component) => ({
    ...
    bestOf: component.bestOf || component.count,
    maxMarks: component.maxMarks,
    weight: component.weight,
    scores: Array.isArray(component.scores) ? component.scores.join(", ") : ""
  }));
}

function officialPlanForTrimester(trimester) {
  if (Number(trimester) <= 6) {
    return [
      { id: makeId(), type: "PT", name: "PT1-PT6", count: 6, bestOf: 5, maxMarks: 30, weight: 90, scores: "" },
      { id: makeId(), type: "NPT", name: "NPT1-NPT6", count: 6, bestOf: 5, maxMarks: 10, weight: 10, scores: "" }
    ];
  }
  return [
    { id: makeId(), type: "NPT", name: "NPT1-NPT12", count: 12, bestOf: 10, maxMarks: 10, weight: 10, scores: "" },
    { id: makeId(), type: "PT", name: "PT1-PT2", count: 2, bestOf: 2, maxMarks: 30, weight: 40, scores: "" },
    { id: makeId(), type: "ST", name: "ST1", count: 1, bestOf: 1, maxMarks: 100, weight: 50, scores: "" }
  ];
}

function inferComponentType(component) {
  const raw = String(component.type || component.name || "").toUpperCase();
  if (raw.includes("NPT")) return "NPT";
...
function renderComponentRows() {
  ...
  $("#componentRows").innerHTML = plan.map((component, index) => `
    <tr>
      <td>
        <select data-index="${index}" data-field="type" aria-label="Assessment type">
          ${["PT", "NPT", "ST", "Custom"].map((type) => `<option value="${type}" ${inferComponentType(component) === type ? "selected" : ""}>${type}</option>`).join("")}
        </select>
      </td>
      <td><input data-index="${index}" data-field="name" value="${escapeAttr(component.name)}" aria-label="Component name"></td>
      <td><input data-index="${index}" data-field="count" type="number" min="1" value="${Number(component.count) || 1}" aria-label="Component count"></td>
      <td><input data-index="${index}" data-field="bestOf" type="number" min="1" value="${Number(component.bestOf) || Number(component.count) || 1}" aria-label="Best of count"></td>
      <td><input data-index="${index}" data-field="maxMarks" type="number" min="1" value="${Number(component.maxMarks) || 1}" aria-label="Maximum marks"></td>
      <td><input data-index="${index}" data-field="weight" type="number" min="0" max="100" step="0.1" value="${Number(component.weight) || 0}" aria-label="Weight percent"></td>
      <td><input class="scores-input" data-index="${index}" data-field="scores" value="${escapeAttr(component.scores || "")}" placeholder="88, 91, 76" aria-label="Scores"></td>
      <td><button class="ghost-btn" type="button" data-remove="${index}">Remove</button></td>
    </tr>
  `).join("");

  $$("#componentRows input, #componentRows select").forEach((input) => {
    input.addEventListener("input", updateAssessmentComponent);
    input.addEventListener("change", updateAssessmentComponent);
  });
  $$("[data-remove]", $("#componentRows")).forEach((button) => {
    button.addEventListener("click", () => removeAssessmentComponent(Number(button.dataset.remove)));
  });
  renderCalculatorSummary();
}
...
function updateAssessmentComponent(event) {
  ...
  const component = state.assessmentPlans[course.code][index];
...
function addAssessmentComponent() {
  const course = getSelectedCourse();
  state.assessmentPlans[course.code].push({
    id: makeId(),
    type: "Custom",
    name: "New Component",
    count: 1,
...
function removeAssessmentComponent(index) {
  const course = getSelectedCourse();
  state.assessmentPlans[course.code].splice(index, 1);
  renderComponentRows();
  persistState();
}
...
function renderCalculatorSummary() {
  ...
  $("#gradePoint").textContent = `${grade.point} grade points`;
  ...
  const target = clamp(Number($("#targetInput").value) || 0, 0, 100);
  ...
  if (result.remainingWeight > 0 && gap > 0) {
    ...
    $("#targetMessage").textContent = "Target is currently covered by projected score.";
  } else {
...
function evaluateComponent(component, componentIndex) {
  ...
  const parsedScores = parseScores(component.scores, maxMarks);
  const items = Array.from({ length: count }, (_, index) => {
    const entered = index < parsedScores.length;
    const score = entered ? parsedScores[index] : 0;
    return {
      componentIndex,
      index,
      label: `${type === "Custom" ? "A" : type}${index + 1}`,
      score,
      maxMarks,
      entered,
...
function renderOfficialBreakdown(result) {
  ...
  $("#engineRuleLabel").textContent = trimester <= 6
    ? "T1-T6: best 5 PT + best 5 NPT"
    : "T7-T9: best 10 NPT + all PT + ST";
  $("#officialBreakdown").innerHTML = result.components.map((component) => `
    <div class="engine-card">
      ...
      <dl>
        <dt>${component.type} %</dt><dd>${formatNumber(component.percentage, 1)}%</dd>
...
function renderWhatIfSummary(result) {
  const targetLetter = $("#whatIfGradeSelect").value || activeGradeScale()[0]?.letter;
  const targetGrade = activeGradeScale().find((grade) => grade.letter === targetLetter) || activeGradeScale()[0];
...
function findRequiredNextAssessmentScore(targetPercentage) {
  ...
  for (let componentIndex = 0; componentIndex < plan.length; componentIndex += 1) {
    ...
    const count = Math.max(1, Number(component.count) || 1);
    const maxMarks = Math.max(1, Number(component.maxMarks) || 1);
    const scores = parseScores(component.scores, maxMarks);
    if (scores.length >= count) continue;
    const missingIndex = scores.length;
    for (let score = 0; score <= maxMarks; score += 0.5) {
      const cloned = clonePlanForSimulation(plan);
      const clonedScores = parseScores(cloned[componentIndex].scores, maxMarks);
      clonedScores[missingIndex] = score;
      cloned[componentIndex].scores = clonedScores.join(", ");
...
function clonePlanForSimulation(plan) {
  return plan.map((component) => ({ ...component }));
}
...
function parseScores(value, maxMarks) {
  return String(value || "")
    .split(",")
    .map((item) => Number(item.trim()))
    .filter((item) => Number.isFinite(item))
    .map((item) => clamp(item, 0, maxMarks));
}
