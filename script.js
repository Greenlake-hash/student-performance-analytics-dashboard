...
function planFromRule(rule, previousPlan = []) {
  ...
  const activeRules = (rule?.components || []).filter((component) => component.enabled !== false);
  const activeTypes = new Set(activeRules.map((component) => normalizeAssessmentType(component.type)));
  const countsByType = new Map();
  definitions
    .filter((definition) => definition.enabled !== false && activeTypes.has(normalizeAssessmentType(definition.type)))
    .forEach((definition) => {
      const type = normalizeAssessmentType(definition.type);
  ...
  return definitions
    ...
    .map((definition, index) => {
      const type = normalizeAssessmentType(definition.type);
      const ruleComponent = activeRules.find((component) => normalizeAssessmentType(component.type) === type);
      const bestOf = Math.max(1, Math.min(Number(ruleComponent?.bestOf) || countsByType.get(type) || 1, countsByType.get(type) || 1));
      ...
      return {
        ...
        maxMarks,
        studentScore,
        weight: ruleComponent ? clamp(Number(ruleComponent.weight) || 0, 0, 100) / bestOf : 0,
...
function renderCourseAssessmentManager() {
  ...
  $("#courseAssessmentRows").innerHTML = definitions.map((assessment, index) => `
    <article class="rule-editor-card course-assessment-editor">
      <div class="rule-editor-head">
        <strong>${escapeHtml(assessment.id)}</strong>
        <label class="mini-toggle">
      ...
      <div class="rule-editor-grid">
        <label>
          Assessment Name
          <input data-course-assessment-index="${index}" data-course-assessment-field="id" value="${escapeAttr(assessment.id)}" aria-label="Assessment name">
        </label>
        <label>
          Type
          <input data-course-assessment-index="${index}" data-course-assessment-field="type" value="${escapeAttr(assessment.type)}" aria-label="Assessment type">
        </label>
        <label>
          Maximum Marks
          <input data-course-assessment-index="${index}" data-course-assessment-field="maxMarks" type="number" min="1" step="0.5" value="${Number(assessment.maxMarks) || 1}" aria-label="${escapeAttr(assessment.id)} maximum marks">
        </label>
      </div>
...
function updateCourseAssessmentDefinition(event) {
  ...
  if (field === "enabled") {
    ...
    assessment.type = normalizeAssessmentType(value);
    ensureComponentDefinition(assessment.type);
  } else if (field === "id") {
...
function addCourseAssessmentDefinition() {
  const course = getSelectedCourse();
  const override = ensureCourseAssessmentOverride(course);
  const type = getActiveRule().components?.[0]?.type || "CUSTOM";
...
function updateAssessmentComponent(event) {
  ...
  const value = event.target.type === "checkbox" ? event.target.checked : event.target.value;
  component[field] = ["bestOf", "weight"].includes(field) ? Number(value) : value;
  ...
  return {
    ...
    courses: Object.fromEntries(
      Object.entries({ ...nestedCourses, ...directCourses }).map(([code, value]) => [
        code,
        normalizeCourseAssessmentSet(value)
      ])
    )
  };
}

function normalizeCourseAssessmentSet(set) {
  return {
    id: set?.id || makeId(),
    appliesTo: {
...
function findPreviousAssessment(previousAssessments, definition, index) {
  const type = normalizeAssessmentType(definition.type);
  return previousAssessments.find((assessment) => assessment.id === definition.id && assessment.type === type)
    || previousAssessments.filter((assessment) => assessment.type === type)[indexWithinType(courseAssessmentDefinitionsForCourse(getSelectedCourse()), definition, index)]
    || null;
}
...
function setMaxMarksAt(component, index, value) {
  component.maxMarks = Math.max(1, Number(value) || 1);
  const course = getSelectedCourse();
  const override = ensureCourseAssessmentOverride(course);
  const matching = override?.assessments?.find((assessment) => assessment.id === component.id && normalizeAssessmentType(assessment.type) === normalizeAssessmentType(component.type));
      ...
      <section class="assessment-group" aria-labelledby="assessmentGroup${componentIndex}">
        ...
        <div class="assessment-card-grid">
          ${component.items.map((item) => {
            ...
            return `
              <article class="assessment-card ${status} ${item.validationError ? "invalid" : ""}" data-status="${status}">
                ...
                <label>
                  ...
                  <input
                    ...
                    min="1"
                    step="0.5"
                    value="${escapeAttr(item.maxMarks)}"
                    data-max-index="${item.planIndex}"
                    aria-label="${escapeAttr(item.label)} maximum marks">
                </label>
                ${item.validationError ? `<div class="validation-message">${escapeHtml(item.validationError)}</div>` : ""}
                <div class="assessment-meta">
                  <span>Weight Contribution</span>
                  <strong>${formatNumber(item.contribution, 2)}%</strong>
                </div>
                <div class="best-status ${status}">
                  ${statusIcon(status)} ${status === "included" ? "Included" : status === "dropped" ? "Dropped (Best-of Rule)" : "Pending"}
                </div>
              </article>
            `;
          }).join("")}
...
function evaluatePlan(plan) {
  const rule = getActiveRule();
  const activeComponents = (rule.components || []).filter((component) => component.enabled !== false);
  const components = activeComponents.map((component, componentIndex) => {
    const type = normalizeAssessmentType(component.type);
    const assessments = (plan || [])
      .map((assessment, planIndex) => ({ ...assessment, __planIndex: planIndex }))
      .filter((assessment) => assessment.enabled !== false && normalizeAssessmentType(assessment.type) === type);
    return evaluateComponent(component, assessments, componentIndex);
  });
...
function evaluateComponent(ruleComponent, assessments, componentIndex) {
  const type = normalizeAssessmentType(ruleComponent.type);
  const count = assessments.length;
  const bestOf = count ? clamp(Math.round(Number(ruleComponent.bestOf) || count), 1, count) : 0;
  const weight = clamp(Number(ruleComponent.weight) || 0, 0, 100);
  const assessmentWeight = bestOf ? weight / bestOf : 0;
  const items = assessments.map((assessment, index) => {
    ...
    const validationError = entered && rawScore > maxMarks ? "Score cannot exceed maximum marks" : "";
    return {
      componentIndex,
      index,
      planIndex: assessment.__planIndex ?? index,
      key: `${type}-${assessment.id}`,
      id: assessment.id,
      label: assessment.id,
      rawScore,
      score,
  ...
  items.forEach((item) => {
    item.contribution = selectedKeys.has(item.key) ? item.percentage * (item.weight / 100) : 0;
  });
  selected.forEach((item) => {
    item.contribution = item.percentage * (item.weight / 100);
  });
  dropped.forEach((item) => {
    item.contribution = 0;
  });
  const contribution = selected.reduce((sum, item) => sum + item.contribution, 0);
  const percentage = weight ? (contribution / weight) * 100 : 0;
